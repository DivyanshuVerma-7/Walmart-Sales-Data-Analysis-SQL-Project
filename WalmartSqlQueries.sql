...
LOAD DATA LOCAL INFILE '/Users/divyanshu/Downloads/Walmart Sales Data.csv'
INTO TABLE sales
...
SET @average_total = (SELECT AVG(total) FROM sales);

UPDATE sales 
SET product_category = 
    CASE 
        WHEN total >= @average_total THEN "Good"
        ELSE "Bad"
    END;
...
WITH ratings_per_time AS (SELECT  branch, time_of_day, AVG(rating) AS average_rating
FROM sales GROUP BY time_of_day, branch ORDER BY branch, average_rating DESC)
SELECT branch, time_of_day, average_rating FROM ratings_per_time
WHERE average_rating IN (SELECT MAX(average_rating) FROM ratings_per_time GROUP BY branch);
...
WITH ratings_per_day AS (SELECT  branch, day_name, AVG(rating) AS average_rating
FROM sales GROUP BY day_name, branch ORDER BY branch, average_rating DESC)
SELECT branch, day_name, average_rating FROM ratings_per_day
WHERE average_rating IN (SELECT MAX(average_rating) FROM ratings_per_day GROUP BY branch);
